...
;; Author: Noacco Francesco
;; URL: <https://github.com/noaccOS/fox-theme>
;;
;; Version: 0.1
;; Keywords: color, theme
;; Package-Requires: ((emacs "24"))

;; Initially created by a fork of spacemacs-theme <https://github.com/syl20bnr/spacemacs-theme>
...
;; Color theme for the fox rice
;; Visit my dotfiles on gitlab <https://gitlab.com/noaccOS/dotfiles>

;;; Code:

(deftheme fox-theme)

(defgroup fox-theme nil
  "Fox-theme options."
  :group 'faces)

(defcustom fox-theme-comment-bg t
  "Use a background for comment lines."
  :type 'boolean
  :group 'fox-theme)

(defcustom fox-theme-comment-italic nil
  "Enable italics for comments and also disable background."
  :type 'boolean
  :group 'fox-theme)

(defcustom fox-theme-keyword-italic nil
  "Enable italics for keywords."
  :type 'boolean
  :group 'fox-theme)

(defcustom fox-theme-org-agenda-height nil
...
(defcustom fox-theme-org-height t
  "Use varying text heights for org headings."
  :type 'boolean
  :group 'fox-theme)

(defcustom fox-theme-org-bold t
  "Inherit text bold for org headings"
  :type 'boolean
  :group 'fox-theme)

(defcustom fox-theme-org-priority-bold t
  "Inherit text bold for priority items in agenda view"
  :type 'boolean
  :group 'fox-theme)

(defcustom fox-theme-org-highlight nil
  "Highlight org headings."
  :type 'boolean
  :group 'fox-theme)

(defcustom fox-theme-custom-colors nil
  "Specify a list of custom colors."
  :type 'alist
  :group 'fox-theme)

(defcustom fox-theme-underline-parens t
...
(let ((class '((class color) (min-colors 89))) ;;              ~~ Dark ~~                              ~~ Light ~~
     ;;                                 GUI       TER
     ;; generic
     (act1          (if (true-color-p) "#3d3451" "#3d3451"))
     (act2          (if (true-color-p) "#3d3451" "#3d3451"))
     (base          (if (true-color-p) "#ffffff" "#ffffff"))
     (base-dim      (if (true-color-p) "#ebebeb" "#ebebeb"))
     (bg1           (if (true-color-p) "#2d253a" "#2d253a"))
     (bg2           (if (true-color-p) "#3e364a" "#3e364a"))
     (bg3           (if (true-color-p) "#4f485a" "#4f485a"))
     (bg4           (if (true-color-p) "#5f5969" "#5f5969"))
     (border        (if (true-color-p) "#b06f8d" "#b06f8d"))
     (cblk          (if (true-color-p) "#ffffff" "#ffffff"))
     (cblk-bg       (if (true-color-p) "#3d3451" "#3d3451"))
     (cblk-ln       (if (true-color-p) "#ebebeb" "#ebebeb"))
     (cblk-ln-bg    (if (true-color-p) "#3d3451" "#3d3451"))
     (const         (if (true-color-p) "#d2bea5" "#d2bea5"))
     (comment       (if (true-color-p) "#d2bea5" "#d2bea5"))
     (comment-light (if (true-color-p) "#d2bea5" "#d2bea5"))
     (comment-bg    (if (true-color-p) "#2d253a" "#2d253a"))
     (comp          (if (true-color-p) "#b06f8d" "#b06f8d"))
     (cursor        (if (true-color-p) "#b06f8d" "#b06f8d"))
     (err           (if (true-color-p) "#f8c083" "#f8c083"))
     (func          (if (true-color-p) "#3a6a9a" "#3a6a9a"))
     (head1         (if (true-color-p) "#f8c083" "#f8c083"))
     (head1-bg      (if (true-color-p) "#b06f8d" "#b06f8d"))
     (head2         (if (true-color-p) "#b06f8d" "#b06f8d"))
     (head2-bg      (if (true-color-p) "#f8c083" "#f8c083"))
     (head3         (if (true-color-p) "#7facbf" "#7facbf"))
     (head3-bg      (if (true-color-p) "#80466c" "#80466c"))
     (head4         (if (true-color-p) "#3a6a9a" "#3a6a9a"))
     (head4-bg      (if (true-color-p) "#3d3451" "#3d3451"))
     (highlight     (if (true-color-p) "#3d3451" "#3d3451"))
     (highlight-dim (if (true-color-p) "#3d3451" "#3d3451"))
     (keyword       (if (true-color-p) "#b06f8d" "#b06f8d"))
     (lnum          (if (true-color-p) "#ebebeb" "#ebebeb"))
     (mat           (if (true-color-p) "#f8c083" "#f8c083"))
     (meta          (if (true-color-p) "#80466c" "#80466c"))
     (str           (if (true-color-p) "#7facbf" "#7facbf"))
     (suc           (if (true-color-p) "#f8c083" "#f8c083"))
     (ttip          (if (true-color-p) "#ffffff" "#ffffff"))
     (ttip-sl       (if (true-color-p) "#f8c083" "#f8c083"))
     (ttip-bg       (if (true-color-p) "#3d3451" "#3d3451"))
     (type          (if (true-color-p) "#10659c" "#10659c"))
     (var           (if (true-color-p) "#f8c083" "#f8c083"))
     (war           (if (true-color-p) "#7facbf" "#7facbf"))

     ;; colors
     ;; WARNING: colors are untouched from spacemacs theme as of this release
  ...
  (cl-loop for (cvar . val) in fox-theme-custom-colors
          do (set cvar val))
...
;;; basics
  ...
  `(font-lock-comment-face ((,class (:foreground ,(if fox-theme-comment-italic comment-light comment) :background ,(when fox-theme-comment-bg comment-bg) :slant ,(if fox-theme-comment-italic 'italic 'normal)))))
  `(font-lock-constant-face ((,class (:foreground ,const))))
  `(font-lock-doc-face ((,class (:foreground ,meta))))
  `(font-lock-function-name-face ((,class (:foreground ,func :inherit bold))))
  `(font-lock-keyword-face ((,class (:inherit bold :foreground ,keyword :slant ,(if fox-theme-keyword-italic 'italic 'normal)))))
...
;;; latex
  ...
  `(font-latex-sectioning-0-face ((,class (:inherit bold :foreground ,head3 :height ,(if fox-theme-org-height 1.3 1.0) :background ,(when fox-theme-org-highlight head3-bg)))))
  `(font-latex-sectioning-1-face ((,class (:inherit bold :foreground ,head4 :height ,(if fox-theme-org-height 1.3 1.0) :background ,(when fox-theme-org-highlight head4-bg)))))
  `(font-latex-sectioning-2-face ((,class (:inherit bold :foreground ,head1 :height ,(if fox-theme-org-height 1.3 1.0) :background ,(when fox-theme-org-highlight head1-bg)))))
  `(font-latex-sectioning-3-face ((,class (:inherit bold :foreground ,head2 :height ,(if fox-theme-org-height 1.2 1.0) :background ,(when fox-theme-org-highlight head2-bg)))))
  `(font-latex-sectioning-4-face ((,class (:bold nil :foreground ,head3 :height ,(if fox-theme-org-height 1.1 1.0) :background ,(when fox-theme-org-highlight head3-bg)))))
  `(font-latex-sectioning-5-face ((,class (:bold nil :foreground ,head4 :background ,(when fox-theme-org-highlight head4-bg)))))
...
;;; markdown
  `(markdown-header-face-1 ((,class (:inherit bold :foreground ,head1 :height ,(if fox-theme-org-height 1.3 1.0) :background ,(when fox-theme-org-highlight head1-bg)))))
  `(markdown-header-face-2 ((,class (:inherit bold :foreground ,head2 :height ,(if fox-theme-org-height 1.2 1.0) :background ,(when fox-theme-org-highlight head2-bg)))))
  `(markdown-header-face-3 ((,class (:bold nil :foreground ,head3 :height ,(if fox-theme-org-height 1.1 1.0) :background ,(when fox-theme-org-highlight head3-bg)))))
  `(markdown-header-face-4 ((,class (:bold nil :foreground ,head4 :background ,(when fox-theme-org-highlight head4-bg)))))
...
;;; org
  ...
  `(org-agenda-date ((,class (:foreground ,var :height ,(if fox-theme-org-agenda-height 1.1 1.0)))))
  `(org-agenda-date-today ((,class (:foreground ,keyword :inherit bold :height ,(if fox-theme-org-agenda-height 1.3 1.0)))))
  `(org-agenda-date-weekend ((,class (:inherit bold :foreground ,var))))
  `(org-agenda-done ((,class (:foreground ,suc :height ,(if fox-theme-org-agenda-height 1.2 1.0)))))
  ...
  `(org-document-title ((,class (:foreground ,func :inherit bold :height ,(if fox-theme-org-height 1.4 1.0) :underline t))))
  ...
  `(org-level-1 ((,class (:inherit bold :bold ,(if fox-theme-org-bold 'unspecified nil) :foreground ,head1 :height ,(if fox-theme-org-height 1.3 1.0) :background ,(when fox-theme-org-highlight head1-bg)))))
  `(org-level-2 ((,class (:inherit bold :bold ,(if fox-theme-org-bold 'unspecified nil) :foreground ,head2 :height ,(if fox-theme-org-height 1.2 1.0) :background ,(when fox-theme-org-highlight head2-bg)))))
  `(org-level-3 ((,class (:bold nil :foreground ,head3 :height ,(if fox-theme-org-height 1.1 1.0) :background ,(when fox-theme-org-highlight head3-bg)))))
  `(org-level-4 ((,class (:bold nil :foreground ,head4 :background ,(when fox-theme-org-highlight head4-bg)))))
  ...
  `(org-priority ((,class (:foreground ,war :inherit bold :bold ,(if fox-theme-org-priority-bold 'unspecified nil)))))
  `(org-quote ((,class (:inherit org-block :slant italic))))
  `(org-scheduled ((,class (:foreground ,comp))))
  `(org-scheduled-today ((,class (:foreground ,func :height ,(if fox-theme-org-agenda-height 1.2 1.0)))))
...
;;; show-paren
  `(show-paren-match ((,class (:foreground ,mat :inherit bold  :underline ,(when fox-theme-underline-parens t)))))
  `(show-paren-match-expression ((,class (:background ,green-bg-s))))
  `(show-paren-mismatch ((,class (:foreground ,err :inherit bold :underline ,(when fox-theme-underline-parens t)))))

;;; smartparens
  `(sp-pair-overlay-face ((,class (:background ,highlight :foreground nil))))
  `(sp-show-pair-match-face ((,class (:foreground ,mat :inherit bold  :underline ,(when fox-theme-underline-parens t)))))
...
;;; fox-specific
  `(fox-transient-state-title-face ((,class (:background nil :foreground ,comp :box nil :inherit bold))))
...
(provide-theme 'fox-theme)

;; Local Variables:
;; no-byte-compile: t
;; End:

;;; fox-theme.el ends here
